...
class Movie2011ListView(GenericAPIView):
    queryset = Movie2011.objects.all()

    serializer_class = Movie2011Serializer  # 指定序列化器类
    permission_classes = [AllowAny]  # 允许所有用户访问

    filter_backends = [DjangoFilterBackend]  # 指定过滤器
    filter_fields = ['id', 'name', 'box']  # 指定需要过滤的字段
...
class Movie2011DetailView(GenericAPIView):
    queryset = Movie2011.objects.all()

    serializer_class = Movie2011Serializer
