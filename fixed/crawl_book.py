...
logging.basicConfig(level=logging.DEBUG,
                    ...
                    filemode='a',
                    filename='/home/interface/fixed/Logs/book.txt')
...
def back_up():
    logging.info('start back up.')
    db = pymysql.connect(host='60.205.201.200',
                         port=3306,
                         user='ping',
                         password='ping',
                         database='interface')
    cursor = db.cursor()
    sql_exists = "SHOW tables like 'book_backup';"
    cursor.execute(sql_exists)
    logging.info(sql_exists)
    info = cursor.fetchall()
    if len(info) == 0:  # 不存在备份的表，就创建备份的表
        sql_backup = 'create table book_backup select * from book;'
        cursor.execute(sql_backup)
        logging.info(sql_backup)
    else:  # 存在备份的表，先删除备份的表，再通过原表内容创建备份的表
        sql_delete = "drop table book;"
        cursor.execute(sql_delete)
        logging.info(sql_delete)
        sql_backup = 'create table book_backup select * from book;'
        cursor.execute(sql_backup)
        logging.info(sql_backup)
    sql_clean = 'truncate table book;'  # 无论存在备份的表，都将执行情况原表的操作
