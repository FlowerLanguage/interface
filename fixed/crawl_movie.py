...
logging.basicConfig(level=logging.INFO,
                    ...
                    filename='/home/interface/fixed/Logs/movie.txt',
                    filemode='a')
headers = {
...
def get_and_post(year_url):
    """
    传入需要爬取的年份url(所有)，调用detail_post，传入单个url
    """
    for i in year_url:
        year = i.split('id=')[1]
        if year == '0':
...
def dispatch():
    ...
    now_time = time.strftime('%Y-%m-%d', time.localtime())  # 提取当前日期
    now_month = now_time.split('-')[1]
    now_day = now_time.split('-')[2]

    if now_month == '01' and now_day == '01':  # 如果是1月1号，则需要单独处理前一年的数据
        year = old[0].split('id=')[1]  # 提取爬取的年份具体数字
        try:
            back_up(year)
        except Exception as e:
            logging.info(e)
            ...
            logging.info('成功执行数据库操作')
            detail_post(old[0])
    else:  # 不是1月1号，则只需要执行可变的年份的更新即可,即使用new
        get_and_post(new)  # 提取并发送可变的，包括当年以及全部电影信息
...
def back_up(year):
    ...
    origin = 'movie_' + year
    sql_exists = "SHOW tables like '{}'".format(origin + '_backup')
    logging.info(sql_exists)
